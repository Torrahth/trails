...
public class ChunkingSystem
{
    ...
    int chunk_rad_X = (int)(4 * Camera.GetZoom());//12;
     int chunk_rad_Y = (int)(3 * Camera.GetZoom());// 10;
    ...
    {
        chunk = new Chunk();
        Console.WriteLine("Register Chunk System");
    }

    public void Update()
    {
        chunk_rad_X = (int)(7 / (2 * Camera.GetZoom())) + 6;
        chunk_rad_Y = (int)(7 / (2 * Camera.GetZoom())) + 2;

        TargetPosition = (Camera.GetPosition() - new Vector2(Global.current_world._sizeX/32, Global.current_world._sizeY/32))/8 ;
        TargetChunk = new Vector2((float)Math.Ceiling(TargetPosition.X /pow ),(float)Math.Ceiling(TargetPosition.Y / pow)) ;
        ...
        for (int x = - chunk_rad_X/2; x < chunk_rad_X/2; ++x)
        {
            ...
            {
               load_chunk(x + (int)TargetChunk.X, y + (int)TargetChunk.Y);
            }
        }
